...
export default function LayerStep2_2({
  ...
  const loadDocumentText = async (docId: string) => {
    ...
    try {
      ...
      // 检查前一个步骤的修改文本（最近的优先）
      // Priority: step2-1 > step2-0 > originalText
      // 优先级：step2-1 > step2-0 > originalText
      const previousSteps = ['step2-1', 'step2-0'];
      ...
      for (const stepName of previousSteps) {
        ...
        if (stepState?.modifiedText) {
          console.log(`[LayerStep2_2] Using modified text from ${stepName}`);
      ...
      if (foundModifiedText) {
        // Use modified text from previous step
        // 使用前一个步骤的修改文本
  ...
  const handleConfirmModify = useCallback(async () => {
    ...
    try {
      ...
      if (sessionId && modifiedText) {
        await substepStore.saveModifiedText('step2-2', modifiedText);
        await substepStore.markCompleted('step2-2');
        console.log('[LayerStep2_2] Saved modified text to substep store');
      }
        ...
        <div className="mb-6 p-6 bg-gray-50 border border-gray-200 rounded-lg">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              Click to analyze section length distribution, CV, and detect extreme lengths
              <br />
              <span className="text-gray-500">点击分析章节长度分布、变异系数和极端长度检测</span>
            </p>
                  ...
                  <div key={section.index} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-medium text-gray-700">
                        {/* Display actual title if available, fallback to role */}
                        {/* 有实际标题时显示标题，否则显示角色 */}
                        {section.title || (section.role.charAt(0).toUpperCase() + section.role.slice(1).replace('_', ' '))}
                        {isKeySection && (
