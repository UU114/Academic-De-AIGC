...
export default function LayerStep2_3({
  ...
  const loadDocumentText = async (docId: string) => {
    ...
    try {
      ...
      const previousSteps = ['step2-2', 'step2-1', 'step2-0'];
      ...
      for (const stepName of previousSteps) {
        ...
        if (stepState?.modifiedText) {
          console.log(`[LayerStep2_3] Using modified text from ${stepName}`);
  ...
  const loadIssueSuggestion = useCallback(async (index: number) => {
    const issue = contentIssues[index];
  ...
  }, [contentIssues, documentId, expandedIssueIndex, loadIssueSuggestion]);
  ...
  const selectAllIssues = useCallback(() => {
    setSelectedIssueIndices(new Set(contentIssues.map((_, idx) => idx)));
  }, [contentIssues]);
  ...
  const handleConfirmModify = useCallback(async () => {
    ...
    try {
      ...
      if (sessionId && modifiedText) {
        await substepStore.saveModifiedText('step2-3', modifiedText);
        await substepStore.markCompleted('step2-3');
        console.log('[LayerStep2_3] Saved modified text to substep store');
      }
        ...
        <div className="mb-6 p-6 bg-gray-50 border border-gray-200 rounded-lg">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              Click to analyze internal structure similarity and detect AI template patterns
              <br />
              <span className="text-gray-500">点击分析内部结构相似性并检测AI模板模式</span>
            </p>
              ...
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                  ...
                  <span className="text-sm font-normal text-gray-500">
                    ({selectedIssueIndices.size}/{contentIssues.length} selected / 已选择)
                  </span>
                </h3>
