...
export default function LayerStep3_4({
  ...
  const loadDocumentText = async (docId: string) => {
    try {
      ...
      // Check previous steps for modified text (step3-3 to step3-0, then Layer 4 steps, then Layer 5 steps)
      // 检查前面步骤的修改文本（先检查step3-3到step3-0，再检查第4层，再检查第5层）
      const previousSteps = ['step3-3', 'step3-2', 'step3-1', 'step3-0',
      ...
      for (const stepName of previousSteps) {
        ...
        if (stepState?.modifiedText) {
          console.log(`[LayerStep3_4] Using modified text from ${stepName}`);
    ...
    try {
      ...
      if (analysisResult.paragraphDetails) {
        analysisResult.paragraphDetails.forEach((para, index) => {
          ...
          // Get preview from paragraph details for better identification
          // 从段落详情获取预览以便更好地识别
          const preview = (para as { preview?: string }).preview || '';
          const locationText = preview
  ...
  const handleConfirmModify = useCallback(async () => {
    ...
    try {
      ...
      if (sessionId && modifiedText) {
        await substepStore.saveModifiedText('step3-4', modifiedText);
        await substepStore.markCompleted('step3-4');
        console.log('[LayerStep3_4] Saved modified text to substep store');
      }
          ...
          <div className="mt-4 p-6 bg-gray-50 border border-gray-200 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Click to analyze sentence length distribution and detect AI patterns
                <br />
                <span className="text-gray-500">点击分析句子长度分布并检测AI模式</span>
              </p>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {/* Show preview for easier identification / 显示预览以便更容易识别 */}
                        {(para as { preview?: string }).preview && (
