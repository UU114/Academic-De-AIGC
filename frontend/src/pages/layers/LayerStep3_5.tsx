...
export default function LayerStep3_5({
  ...
  const loadDocumentText = async (docId: string) => {
    try {
      ...
      // Check previous steps for modified text (step3-4 to step3-0, then Layer 4 steps, then Layer 5 steps)
      // 检查前面步骤的修改文本（先检查step3-4到step3-0，再检查第4层，再检查第5层）
      const previousSteps = ['step3-4', 'step3-3', 'step3-2', 'step3-1', 'step3-0',
      ...
      for (const stepName of previousSteps) {
        ...
        if (stepState?.modifiedText) {
          console.log(`[LayerStep3_5] Using modified text from ${stepName}`);
  ...
  const handleConfirmModify = useCallback(async () => {
    ...
    try {
      ...
      if (sessionId && modifiedText) {
        await substepStore.saveModifiedText('step3-5', modifiedText);
        await substepStore.markCompleted('step3-5');
        console.log('[LayerStep3_5] Saved modified text to substep store');
      }

      // Navigate to Layer 2 Step 4.0 (Sentence Layer)
      // 导航到 Layer 2 Step 4.0（句子层）
          ...
          <div className="mt-4 p-6 bg-gray-50 border border-gray-200 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Click to analyze paragraph transitions and detect formulaic patterns
                <br />
                <span className="text-gray-500">点击分析段落过渡并检测公式化模式</span>
              </p>
