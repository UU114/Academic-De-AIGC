...
async def identify_sections(request: SubstepBaseRequest):
    ...
    try:
        ...
        # Use unified text parsing service for paragraph count
        # 使用统一文本解析服务获取段落数
        ...
        return SectionIdentifyResponse(
            ...
            sections=sections,
            section_boundaries=[0, para_count],
...
async def apply_modification(
    ...
    try:
        ...
        document_text, locked_terms = await get_working_text(
            db, request.session_id, "step2-0", request.document_id
        )
        ...
        if modified_text and request.session_id:
            await save_modified_text(
                db, request.session_id, "step2-0", modified_text
            )
