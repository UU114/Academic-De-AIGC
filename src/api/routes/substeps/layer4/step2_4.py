...
Analyze section transitions using LLM.
使用LLM分析章节衔接与过渡。
"""
...
handler = Step2_4Handler()


@router.post("/analyze", response_model=SectionTransitionResponse)
async def analyze_section_transition(request: SubstepBaseRequest):
    """
    Step 2.4: Analyze section transitions (NOW WITH LLM!)
    步骤 2.4：分析章节衔接与过渡（现在使用LLM！）
    """
    ...
    try:
        ...
        return SectionTransitionResponse(
            ...
            strong_transition_count=result.get("strong_transition_count", 0),
            weak_transition_count=result.get("weak_transition_count", 0),
            semantic_echo_score=result.get("semantic_echo_score", 0)
        )

    except Exception as e:
        logger.error(f"Section transition analysis failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/merge-modify/prompt", response_model=MergeModifyPromptResponse)
async def generate_prompt(request: MergeModifyRequest):
    """Generate modification prompt for section transition issues"""
    try:
...
async def apply_modification(
    ...
    """Apply AI modification for section transition issues"""
    try:
        # Get working text using document_service
        document_text, locked_terms = await get_working_text(
            db, request.session_id, "step2-4", request.document_id
        )
        ...
        if modified_text and request.session_id:
            await save_modified_text(
                db, request.session_id, "step2-4", modified_text
            )
