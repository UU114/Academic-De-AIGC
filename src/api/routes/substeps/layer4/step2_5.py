...
Analyze inter-section logic using LLM.
使用LLM分析章节间逻辑关系。
"""
...
async def analyze_inter_section_logic(request: SubstepBaseRequest):
    """
    Step 2.5: Analyze inter-section logic (NOW WITH LLM!)
    步骤 2.5：分析章节间逻辑关系（现在使用LLM！）
    """
    ...
    try:
        ...
        return InterSectionLogicResponse(
            ...
            argument_chain_score=result.get("argument_chain_score", 0),
            redundancy_score=result.get("redundancy_score", 0),
            progression_linearity=result.get("progression_linearity", 0),
            logic_gaps=result.get("logic_gaps", [])
        )

    except Exception as e:
        logger.error(f"Inter-section logic analysis failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/merge-modify/prompt", response_model=MergeModifyPromptResponse)
async def generate_prompt(request: MergeModifyRequest):
    """Generate modification prompt for inter-section logic issues"""
    try:
