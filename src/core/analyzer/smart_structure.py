...
### 1. DETECT EXPLICIT CONNECTOR WORDS (CRITICAL AI SIGNAL!)
...
- Whether it's at paragraph_start or sentence_start
- severity: high (paragraph start) or medium (sentence start)
...
- **rewrite_example** (Optional - in English):
  ...
  Example: "The escalating trend of soil salinization poses new challenges to traditional agriculture. In the North China Plain, monitoring data from the past decade reveals..."
...
class SmartStructureAnalyzer:
    ...
    async def analyze_structure(self, document_text: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Structure analysis result with sections, paragraphs, and structure issues
        """
        try:
            logger.info("[SmartStructureAnalyzer] Starting Step 1-1: Structure Analysis")
            ...
            # 构建结构分析提示词
            prompt = STRUCTURE_ANALYSIS_PROMPT.format(document_text=document_text)
